...
class Bucket (val size : Size) {
    private val data : MutableList<MutableList<Cell>>
    init {
        data = (0..size.height - 1).map { (0..size.width - 1).map { Cell(isFull = false)}.toMutableList() }.toMutableList()
    }
    operator fun get(p : P2)  = data[p.y][p.x]
    operator fun get(x : Int, y : Int)  = data[y][x]
    operator fun set(p : P2, value : Cell) { data[p.y][p.x] = value }
...
class TetrisScreen(val game : TetrisGame) : Screen, InputProcessor {
    ...
    val font = BitmapFont()
    val random = Random()
    val bucketSize = Size(width = 10, height = 20)
    val cellPixelSize = Size(width = 32, height = 32)
    val shapes = listOf(
...
fun translate(shape : List<P2>, x : Int = 0, y : Int = 0) = shape.map { P2(it.x + x, it.y + y)}
...
fun drawShape(shapeRenderer: ShapeRenderer, shape : Shape, color : Color, cellPixelSize : Size, offset : Float) {
...
fun setPosition(shape : List<P2>, p : P2) : List<P2> {
    val middle = shape[0]
    val mx = p.x - middle.x
    val my = p.y - middle.y
    return shape.map { P2(it.x + mx, it.y + my )}
}
...
fun drawBucket(shapeRenderer: ShapeRenderer, bucket : Bucket, x : Float, y : Float, cellSize : Size) {
    for(i in 0..bucket.size.width-1) {
        for(j in 0..bucket.size.height-1) {
            val b = bucket[i,j]
            when(b.isFull) {
                true -> {
                    drawBlock(shapeRenderer, i * cellSize.width + x , j * cellSize.height + y, cellSize.width.toFloat(), cellSize.height.toFloat(), b.color)
                }
                false -> {}
            }
        }
